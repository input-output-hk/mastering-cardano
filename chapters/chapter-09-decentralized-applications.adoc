...
Now that we have some test cryptocurrency in our wallet, let's try to use a DApp. We'll use a decentralized exchange(((decentralized exchange))) (DEX) to swap some of our testnet Ada for some other token. More specifically we'll use a preprod test network version of the Minswap(((Minswap))) DEX(((DEX))) as follows:
...
[start=5]
. The DApp is likely automatically loaded with details to swap from Ada (which you should have in your wallet) to Min (Minswap(((Minswap)))'s own token). The testnet version of Minswap(((Minswap))) only supports swapping between Ada and Min. When you use the mainnet's version though you can choose to swap to other tokens as well. Enter an amount of Ada that you will swap in from your wallet, and the amount of Min that will be swapped out will be displayed (<<fig-minswap-review-trade>> depicts a swap of 123 test Ada to the relevant amount of test Min at the time of writing).
...
* Client-side code: This is the code that will be sent from the server (discussed above), but will execute in the client-side(((client-side))) browser. This code will connect to the wallet and communicate with a deployed on-chain script.

We will not create on-chain script code in this section (since that is handled in #Chapter 8#). Indeed, DApps(((DApps))) can be created that communicate with existing deployed on-chain scripts that may not necessarily be written by the same developers/teams -- just as we demonstrate now below.

_Creating a Server (with NodeJS)_

We now discuss creating a NodeJS(((NodeJS))) server that will be used to serve content to requesting users. You can use any other framework to create server-side(((server-side))) code if you wish (such as Python, PHP, .NET, Java, or any other framework you may prefer). We'll use NodeJS(((NodeJS)))' express package. Follow these steps to create the server:

. First, you need to ensure that NodeJS(((NodeJS))) is installed, and that you can run 'node' and 'npm' from the command line.
...
and for ease of this tutorial, you can just keep all default settings. +
 +
 This will create a package.json file that defines the settings of the NodeJS(((NodeJS))) project. Verify that the 'main' setting is set to 'index.js'. This setting defines the main entry point file for code in the NodeJS(((NodeJS))) project.
...
Now that we have a server able to send HTML/JavaScript to end-users, let's write the client-side(((client-side))) code to connect to a user's wallet and interact with the underlying on-chain scripts. We'll only provide the bare minimal code that is needed. Indeed, you may want to look into implementing a full HTML page (including html, head and body tags), but we'll only provide the necessities for the sake of simplicity.
...
. Install webpack(((webpack))) and webpack(((webpack)))-cli by running:
[source]
----
...
[start=3]
. In the NodeJS(((NodeJS))) application, create the file ./mesh-entry.js with the following code:
...
[start=4]
. Create the ./webpack(((webpack))).config.js file with the following code:
...
[start=5]
. Run webpack(((webpack))) to generate the bundled Mesh library:
[source]
----
npx webpack
----
...
. The NodeJS(((NodeJS))) ./index.js application should be updated to allow for the bundled Mesh library to be served to clients by adding the following line:
...
==== Interacting with the Redeemer 42(((Redeemer 42))) On-Chain Script Code

To demonstrate DApp interaction, we'll write client-side JavaScript code to interact with the Redeemer 42(((Redeemer 42))) on-chain script code (discussed in #Chapter 8#).footnote:[Also see https://github.com/LukaKurnjek/ppp-plutusV3-plinth/blob/main/off-chain/meshjs/Week02/redeemer42-ref-script.ts] You can read Section <<Simple validation scripts>> to get a better understanding of the Redeemer 42(((Redeemer 42))) Script (if you have not already done so). We'll send funds, deploy a reference script and then claim back the funds sent.

The Redeemer 42(((Redeemer 42)))'s reference script that the DApp will interact with has already been deployed to the preprod network. Its transaction hash is: ac43f379762d68839a75d95146c332e6025e5a305fffc071308d138849109bfc



_Sending Funds to the Redeemer 42(((Redeemer 42))) On-chain Scripts_

To add functionality that sends funds to the Redeemer 42(((Redeemer 42))) on-chain script code follow these steps:
...
[start=3]
. Add into the client-side JavaScript code the following to get a reference to the deployed Redeemer 42(((Redeemer 42))) script:
...
After running the NodeJS(((NodeJS))) server and refreshing the page (i.e. refreshing localhost:3000), you should see the added button 'Send Funds':
...
_Deploying a reference script for the Redeemer 42(((Redeemer 42))) example_

To add functionality that deploys a reference script (discussed in #Chapter 8#) for the Redeemer 42(((Redeemer 42))) example follow these steps:
...
In Line 3-5, we create an instance of a transaction builder that will we use in a few lines.
...
After re-running the NodeJS(((NodeJS))) server and refreshing the page (i.e. refreshing localhost:3000), you should see the added button 'Deploy Reference Script':
...
_Claiming back funds from the Redeemer 42(((Redeemer 42))) example_
...
In Line 7, we define the reedemer value (42) to send to the script.
...
After re-running the NodeJS(((NodeJS))) server and refreshing the page (i.e. refreshing localhost:3000), you should see the added button 'Claim Funds':
...
=== DApps(((DApps))) and UI/UX Issues

While DApps(((DApps))) promise to decentralize many multi-party digital services, without a doubt there are still several challenges that must be overcome for their mass-adoption -- particularly for the non-tech-savvy. We now discuss some challenges (that DApps(((DApps))) on all blockchains face) and potential future directions to overcome such challenges:
...
* *Gas costs*: for non-cryptocurrency related DApps(((DApps))), esppecially those that a user may not interact with often, users may find it troublesome to both purchase and cover required gas costs. Solutions to this may include feeless/gasless transaction models.
...
* *Losing access to keys*: Whilst not exactly a barrier to using DApps(((DApps))), some users may not feel comfortable using DApps(((DApps))) associated with a private key/seed phrase that they may lose. Different approaches are being investigated to circumvent users taking full responsibility on keeping their keys safe including: social recovery, shared wallets, and multi-sig wallets.
