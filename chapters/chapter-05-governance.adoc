...
1. *Registration*. The registration process involves taking a snapshot of the participant’s ada holdings at a specific point in time. This snapshot determines the voting power of each participant.
2. *Download* the https://projectcatalyst.io/get-involved/become-a-voter[Catalyst Voting App], available on mobile devices.
3.*Reviewing proposals*. All proposals are publicly accessible on the IdeaScale platform and the voting application. Detailed information about each proposal, including the problem statement, proposed solution, team details, and budget is there to discuss and provide feedback on.
4. *Voting begins*. For each proposal, voters typically have multiple options to express their support or opposition.Once you’ve cast a vote on blockchain, you cannot change it anymore.
5. *Counting votes*. After the voting period ends, votes are tallied.
6. *Announcing results*. The results are announced publicly, detailing which proposals have been selected for funding. This transparency helps build trust within the community.
...
Project Catalyst constantly improves with each iteration, leveraging support from it’s partners. For example, Funded Projects from Funds 9–13 can now avail of legals support from Storm Partners. This may include: 
