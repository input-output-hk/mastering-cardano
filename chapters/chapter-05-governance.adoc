...
CIPs are processed in a semi-formal manner. Editors of CIP proposals(((proposals))) meet regularly to discuss and evaluate ideas. The biweekly meeting http://github.com/cardano-foundation/CIPs/tree/master/BiweeklyMeetings[minutes] are publicly available.
...
3. *Voting*: any ada holder can vote on proposals using the https://cardano.ideascale.com/[Catalyst voting application(((Catalyst, voting application)))]. Voting power is proportional to the amount of ada held, incentivizing ada holders to participate actively in the governance process.
...
The second and final stage of the Chang upgrade, the Plomin hard fork occurred on the 29th January 2025. Cardano’s move to the Voltaire development phase(((Cardano, move to the Voltaire development phase))) was now complete and Cardano was henceforth to be fully governed by the community.
...
As Intersect is the only Administrator available at time of writing, it initiates all actions, with an Oversight Committee scrutinizing each step(((Oversight Committee scrutinizing each step))). Intersect apply various best practices to ensure the integrity of governance actions. For example, they published the verification key used to sign all governance metadata they author. This is an important safeguard which prevents misuse and ensures accountability. This is enabled by governance metadata standards defined in CIP-100 and CIP-108. 

*Oversight Committee*

The Oversight Committee is composed of independent third-parties who provides an additional verification layer within the smart contract framework. They cross-check the accuracy and consistency(((cross-check the accuracy and consistency))) of key actions. The inaugural members are:
