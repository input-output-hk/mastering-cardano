...
Let us look again at the Haskell definition of the [purple]#Value# type. 
...
Let us now look at the unit test for this contract that uses the PSM
...
Let us now explore helper functions:
...
transaction. Let us now check from the REPL which types the [blue]#payToScript#
function accepts:
...
datum. Let us now look at the [blue]#testScript# function that defines our actual
test:
...
use it to test any code, not just smart contracts. Let us now look at our
...
Let us now look at the helper functions we will use in the testing
function:
...
Let us now look at how we define our property functions for testing:
...
Let us now look at the testing functions:
...
Now, Let us examine an example Plinth script that defines a stake
...
in the following 
https://www.essentialcardano.io/article/concurrency-and-all-that-cardano-smart-contracts-and-the-eutxo-model[IO blog]
...
liquidity providers can be larger than the amount of impermanent loss. This 
...
In addition to section <<The EUTXO model>> we list further learning 
resources about the EUTXO model: 
...
security features and best practices of the Plutus and Marlowe smart 
contract languages are further discussed in the remaining sections of 
this chapter. Because those languages follow different best practices 
for developing smart contracts, we will explain the security advantages 
and best practices for them separately. 
