...
language embedded in Haskell which compiles to Untyped Plutus Core (UPLC). Because 
...
We note that Cardano's hard fork combinator technology, that was briefly covered 
in section <<Cardano node layers>>, subsection *Consensus and storage layer*, allows 
the Cardano node to process older Plutus version scripts also after a hard fork 
...
definitions. As already mention all Plinth validator code presented in this 
...
Other auditing companies might not have their auditing reports grouped 
into a single repository and are also worth looking into. 
