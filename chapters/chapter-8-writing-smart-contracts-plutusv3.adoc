...
examples and in addition also contains Lucid Evolution code examples. 
...
contracts for Cardano. Plinth as a high assurance language embeded in Haskell 
...
<<Smart contract programming languages>>, which lists each type and provides 
...
PlutusV3 that became available after the Chang hard fork. From the 
...
https://wiki.haskell.org/Language_Pragmas[Haskell wiki] that provides 
