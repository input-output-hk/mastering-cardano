...
Turing-complete (see <<Glossary>>) smart contracts on Cardano. It 
...
Plutus Core (UPLC). The compilation pipeline and other Plutus security details
...
We also provide a link to a repository that contains in addition to 
the presented MeshJS code examples also Lucid Evolution code examples. 
...
language by showcasing how to write and interact with Plinth smart 
...
that has easy to follow and comprehensive documentation and provides a simple 
setup of the development environment needed to write, test and compile smart 
contracts for Cardano. For developers that want to learn how to code smart 
contracts in Cardano with ease and want to avoid learning Haskell, Aiken is 
a good choice. Plinth as a high assurance language embeded in Haskell works 
better with the https://wiki.portal.chalmers.se/agda/pmwiki.php[Agda] 
proof assistants, that can be used for formal verification of smart contract 
code. A more detailed comparison of Aiken and Plinth can be found in the 
<<Plinth in comparison to Aiken>> section, that also lists Aiken learning 
resources. 
...
As already stated, Plinth is embedded in the Haskell programming language. 
...
off-chain code examples. Further learning resources and smart contract examples 
...
We note that Cardano's hard fork combinator technology, presented in section 
<<Cardano node layers>>, subsection *Consensus and storage layer*, allows the 
Cardano node to process older Plutus version scripts also after a hard fork 
makes a new Plutus script version available. 
...
A UTXO can have an arbitrary piece of data attached to it that we call the 
datum which can define the state of the UTXO. When a transaction intends to spend 
...
every UTXO necessarily needs to contain a datum. The information in the datum 
can be used for: 
...
Cardano CLI or a off-chain code library to check whether a transaction will 
...
off-chain. We call this property determinism of transactions. This does not mean 
...
run a full Cardano node to test their work. The tools that allows testing 
...
language that is written in Rust and directly compiles to Untyped Plutus 
Core (UPLC). This process is illustrated on the diagram below: 
...
syntax to developers, that makes it easy to learn. Aiken enables 
...
Both Plinth and Aiken compile to Plutus, so the logic of writing 
...
Greene provides comprehensive information and learning examples on Aiken. 
...
https://wiki.haskell.org/Language_Pragmas[Haskell wiki] page. The 
https://downloads.haskell.org/ghc/latest/docs/users_guide/using-optimisation.html#ghc-flag--ffull-laziness[GHC docs]
also cover various GHC compiler options. Next we name the module `Week02.Validators` 
because the code comes from the _Week02_ examples of the fourth Plutus pioneer 
program. Then, we import several submodules from the `PlutusLedgerApi` and the 
...
section and also update the `GenBlueprint.hs` file with additional blueprint 
definitions. All validators presented in this chapter can be found at the 
https://github.com/input-output-hk/plutus-pioneer-program/tree/plinth-plutusV3[plinth-plutusV3] 
branch of the Plutus pioneer program, that also contains a `blueprint.json` file 
with all compiled validator code. 
