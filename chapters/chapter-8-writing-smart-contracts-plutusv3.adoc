...
The logic in all compiled Cardano smart contract languages follow the
...
Haskell, Aiken is a good choice. Aiken learning resources are 
...
about transaction being validated. This includes all transaction inputs and 
outputs and other transaction data. A UTXO can have an arbitrary piece of data 
attached to it that we call the datum and defines a state of the UTXO. When a 
transaction intends to spend a UTXO sitting at a script address, the script 
code can access the information contained in the datum that is potentially 
included in the script context. Not every UTXO necessarily needs to contain a 
datum. The information in the datum can be used for: 

* Answering who/when/with wath can I consume a UTXO. 
...
UTXO individually instead of a global state that smart contracts in Ethereum access. 
...
<<Cardano security>> that highlights some of the security advantages that 
Cardano's EUTXO model brings over Ethereum's EVM. You can find research a
rticles about the technical implementation of Plutus in the 
