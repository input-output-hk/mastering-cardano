...
The subsection <<Testing Plinth scripts>> showcases the Atlas tool. 
...
environment for the first three options. An
...
repository. We used nix version `2.25.3` and a nix 
...
validator blueprint definitions for two validators that we have choosen.  
...
token types directly at the protocol level. The key advantages of native 
tokens in Cardano over traditional smart contract-based approaches are:  
...
in the <<Cardano security>> section. Before we look at an example of a mintin 
...
At the end we note that the utility of a token can be determined by its use 
case, the market, or the community that issues and adopts it. Tokens can be 
used for varous purposes as issuing a new cryptocurrency, representing access 
...
Because this kind of attacks are possible in Ethereum, it can make the effect 
of a transaction on the ledger, hard to predict. Such an attack happened 
to the https://en.wikipedia.org/wiki/The_DAO[DAO smart contract] that was 
