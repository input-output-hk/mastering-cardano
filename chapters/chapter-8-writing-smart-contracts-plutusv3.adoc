...
A Bitcoin Script is a simple stack-based smart contract language, who's
...
* Answering who/when/with what can I consume a UTXO. 
...
implementation design of the contract since state is tracked for every UTXO 
individually, instead of having a global state that smart contracts access. 
...
sections contained in the <<Plutus smart contracts>> section focus on 
Plinth data types defined for PlutusV3 scripts. A validation script or 
...
be packed into common lists and maps associated with an integer. Next, we show 
the high-level implementation of the script context data type:  
...
<<Script context explained>>. Let us now examine a simple untyped validator 
