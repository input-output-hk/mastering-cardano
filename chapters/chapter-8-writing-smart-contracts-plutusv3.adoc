...
library for off-chain code, which is further maintained by the
...
<<Off-chain code with MeshJS>> and <<Minting policies>>. The subsection 
<<Testing Plinth scripts>> showcases the Atlas tool. 
...
Section _Plutus smart contracts_ presents the rules for the Plutus 
language by showcasing how to write, interact with, and test Plinth smart 
contracts (formerly called PlutusTx) that is a high-level smart contract 
language embedded in Haskell which compiles to untyped Plutus core. Because 
Plutus rules apply to other compiled languages like Aiken, Plu-Ts or OpShin, 
learning the basics of Plinth helps developers read and write code in other 
Cardano smart contract languages that compile to Plutus. 
...
Cardano ecosystem report] shows that Aiken is by far the most used smart 
contract language in Cardano. It is a user-friendly standalone language that 
has easy to follow and comprehensive documentation and provides a simple 
setup of the development environment needed to write, test and compile  
smart contracts for Cardano. The reason why the _Plutus smart contracts_ 
section focuses on Plinth is because there are already many great learning resources 
for Aiken. Plinth as a high assurance language embeded in Haskell works better 
with the https://wiki.portal.chalmers.se/agda/pmwiki.php[Agda] proof assistants,
that can be used for formal verification of smart contract code. A more 
detailed comparison of Aiken and Plinth can be found in the 
<<Plinth in comparison to Aiken>> section. For developers that want to learn 
how to code smart contracts in Cardano with ease and want to avoid learning 
Haskell, Aiken is the most recommended choice. Aiken learning resources are 
presented at the end of section <<Plinth in comparison to Aiken>>. 
...
in section <<The EUTXO model>>. One should also be familiar with the types 
of smart contract languages offered by Cardano, covered in section 
<<Smart contract programming languages>>, which lists each type and provides resources 
for development tools. Additionally, understanding Cardano addresses and their 
binary format, as explained in section <<Cardano addresses>>, is important. 

As stated in the beginning Plinth is embedded in the Haskell programming language. 
For more details on Haskell, refer to section <<Features and benefits of Haskell>>. 
IO offers a free self-paced beginner-friendly
...
the advantages they bring. The _Plutus smart contracts_ section covers 
...
iteration of the Plutus Pioneer program] and are updated to accommodate PlutusV3
changes. The 4th iteration presents PlutusV2 code examples and in addition  
contains a stablecoin example that is not covered in this book. 
Further learning resources and smart contract examples in Plinth, Aiken 
and other smart contract languages can be found at: 
...
Commercial training for developers interested in building on Cardano is 
offered by https://www.emurgo.io/[Emurgo]. At the time of writing, the 
...
The https://github.com/johnnygreeney/cardanoforthemasses[Cardano for the Masses]
online book written by John Green is also available under an open-source license. 
Beside that the Cardano foundation also offers a free online course called the 
https://academy.cardanofoundation.org/[Cardano academy] that teaches the basics 
of blockchain and Cardano. 

We also note that Cardano’s hard fork combinator technology, presented in section 
<<Cardano node layers>>, subsection *Consensus and storage layer*, allows the 
Cardano node to process older Plutus version scripts also after a hard fork makes 
a new Plutus script version available. If you encounter Plinth code examples written 
in PlutusV1 or PlutusV2 you can look at videos and learning examples of the previous 
Plutus pioneer program iterations: 

* https://github.com/input-output-hk/plutus-pioneer-program/tree/first-iteration[first-itaration]
* https://github.com/input-output-hk/plutus-pioneer-program/tree/second-iteration[second-itaration]
* https://github.com/input-output-hk/plutus-pioneer-program/tree/third-iteration[third-iteration]
...
covers the security aspect of both Plutus and Plinth languages in more detail. 
...
This means a Plutus script can see the script context that carries 
information about transaction being validated. This includes all transaction 
inputs and outputs, all scripts that are being triggered by the transaction 
and other transaction data. Part of this data is the datum and the redeemer. 
A UTXO can have an arbitrary piece of data attached to it that we call the datum 
and defines a state of the UTXO. When a transaction intends to spend a UTXO 
sitting at a script address, the script code can access the information 
contained in the datum that is potentially included in the script context. 
Not every UTXO necessarily needs to contain a datum. The information in the 
datum can be used for: 

* Answering who/when/with watch can I consume a UTXO. 
...
in the script context of a transaction. The information in the redeemer can 
be used for: 

* Defining the reason for interacting with a script as eg request for a loan, 
making a bet, pay a fee, cancel a lone, etc. 
* Provide information to the script known only by person X, which can 
...
Section <<Script context explained>> covers in more detail the information 
contained in the script context that a transaction interacting with a script 
can access. A Plutus script can use all these information to decide whether it 
is ok to consume an UTXO or not. It is also possible to use a tool like the 
Cardano CLI or a wallet to check whether a transaction will be validated 
before on-chain execution. If the transaction is valid, it will be processed 
on the network, provided that all UTXO inputs are present and the processing 
time falls within the transaction’s validity interval. In case these 
conditions are not met, the transaction will fail without charging any fees 
to the user, unlike in Ethereum, where users must pay gas fees for transactions 
that can ultimately fail. If the inputs are present when the transaction is 
submitted, it will be processed and produce the expected outcome as predicted 
off-chain. We call this property determinism of transactions. It doesn't mean 
we know for sure if a transaction will be processed because a UTXO input might 
be consumed by a another transaction just before our transaction gets processed. 
...
powerful in expressing smart contract logic. Translating a smart contract 
from Ethereum to Cardano is in principle possible but requires rethinking the 
implementation design of the contract since state is tracked for every 
UTXO individually instead of a global state that smart contracts access. 
Translating the exact logic of an Ethereum smart contract to Cardano in not 
necessarily good practice, because the end goal is not the smart contract
itself, but the service/business it generates. A better approach is  
to analyze the contract and produce a high-level specification of what it 
does. Then a developer can create a new architecture based on that specification 
to have the same intended properties as the original contract has. 

Further comparison between Cardano and Ethereum is outlined in section 
<<Cardano security>>. You can find research articles about the technical 
implementation of Plutus in the https://iohk.io/en/research/library/[IO library]. 
A good entry point is the research paper
...
or more preceisly to untyped Plutus core (UPLC) which the Cardano nodes execute. 
Plinth provides a compiler plugin for GHC (the Glasgow Haskell Compiler) that 
...
language that is written in Rust and directly compiles to UPLC. This process is
...
Aiken was developed by Lucas Rosa and other developers with the 
support of https://txpipe.io/[TxPipe] and the https://cardanofoundation.org/[Cardano Foundation]. 
...
https://aiken-lang.org/credits[offcial page]. Aiken takes inspiration 
...
utilize the full range of Haskell’s type system features, such as type classes and
https://blog.ocharles.org.uk/guest-posts/2014-12-18-rank-n-types.html[rank-N polymorphism]. 
...
versions (V1, V2 and V3) that allows for better code optimization. 
The https://plutus.cardano.intersectmbo.org/docs/working-with-scripts/optimizing-scripts-with-asData[Plutus user guide] 
provides instructions on that. Aiken compiles all its custom data types 
into the builtin Plutus type [purple]#Data#. We will look at this data 
type in section <<Simple validation scripts>>. 

Both languages are purely functional, which makes it easier for an auditor 
to assess the behavior of such smart contracts. Because Plinth is embedded 
in Haskell, it also works well with proof-assistant tools such as 
...
Plinth and Agda. Both languages compile to Plutus, so the logic of
writing smart contracts in them is very similar, even though the syntax 
is different. Learning one language will help a developer understand any
...
Aiken repository. For those which prefer to learn by a book the 
https://book.io/book/i-can-aiken/[I can Aiken] book, written by John 
Green provides comprehensive information and learning examples on Aiken. 
...
The _plinth-dev-env_ GitHub repository provides instructions for
setting up the development environment for the first three options. An
up-to-date link to this repository is available in the _resources_ folder of the
...
_plinth-dev-env_ repository. We used nix version `2.25.3` and a nix 
shell provided by the https://github.com/IntersectMBO/plinth-template[plinth-template]
...
or type class that you are querying. This is because implementations 
...
context. It contains the following information: 
...
only work in off-chain code but not in on-chain code. The [purple]#Data# 
type has its constructor exposed, as illustrated in the code below:
...
prelude where all functions use strict evaluation rather than the lazy one. 
This also applies to all other functions defined in any module used for 
Plinth coding. We name the module `Week02.Validators` because the code 
comes from the _Week02_ examples of the fourth Plutus pioneer program. 
Then, we import several submodules from the `PlutusLedgerApi` and the 
...
function which takes a value of type [purple]#Bool#, and it returns a value of type 
[purple]#BuiltinUnit# in case the input argument is true or raises an error if it is false. 
...
inlinable. By adding the [pruple]#INLINABLE# pragma statement before or 
...
The <<Plutus smart contracts>> section focuses on data types defined 
for PlutusV3 scripts. The script context contains the transaction information, the 
...
At the beginning we define the custom data type [purple]#MySillyRedeemer# 
...
that requires to add two single quotes in front of the type that return the 
...
Since Plutus core now provides a way to deal directly with the [purple]#Data# type 
...
We first define all language pragmas and import necessary modules including the 
...
that we have used in the validator code, beside the types of the validator type 
signature. After that, we see the [blue]#preamble# definition. Next we show the 
validator blueprint definitions. We show only definitions for two validators 
instead of the six we have defined in this section to shorten the output. 
...
For every validator we can use boilerplate code to define a blueprint. 
...
module that we have imported. One can simply extend the blueprint code such that 
it generates the data and compiled code for all six validators we have defined 
in this section. Below is an example blueprint for the above code. The compiled 
validator code is contained in the `"compiledCode"` fields. 
...
archive. The `cabal` tool can then download the Plutus packages that are defined 
in the `.cabal` file when compiling the project. 
...
In the `.cabal` we define our compiler options, the library where our validator
code resides and the build depends that lists Plutus packages needed by our validator 
code. At the end we define the executable project which tells cabal which file is the 
entry point for compiling the project, and we list also here the build depends. We name 
the file with the code for generating blueprints `GenBlueprint.hs`. Our project should 
be structured the following way:
...
section. So it can be reused when compiling code from other sections that we 
will present next. One only needs to add the module names under the `exposed-modules:`
section and also update the `GenBlueprint.hs` file with additional blueprint definitions. 

==== Script context explained

We look again at the PlutusV3 [purple]#ScriptContext# data type. 
...
If a transaction tries to spend multiple UTXOs at a script address, the 
spending script is run for every UTXO individually. The redeemer and the 
script information are individually assigned for every UTXO in that transaction. 
The transaction information is only one and is accessible to every script 
that is being triggered by a single transaction. 

*Script information* 

In the previous code examples, the transaction information and script information 
were not used in the validation logic. However, in most validator scripts, they are 
used. Let us look at the [purple]#ScriptInfo# data type. 
...
The script information contains information about the script purpose. 
If a transaction tries to mint a native asset the then the script info 
...
The currency symbol data type is a wrapped around a [purple]#BuiltinByteString#
that represents the hash of the minting policy that gets triggered when minting 
a native asset. We will talk more about native assets and minting in section 
<<Minting policies>>. 

If a transaction tries to spend a UTXO at a script address then the script 
...
Each UTXO a transaction creates gets a transaction index assigned starting with 
0 and increasing by 1. Because transaction hashes (IDs) are unique, a UTXO defined 
by the transaction hash and the output index is also unique and can exist only once. 
The reason transaction hashes are unique is because a new transaction hash includes 
the hash of the transaction that created the UTXO being consumed. Every transaction is
consuming at least one UTXO as it needs to pay some fees. By a simple induction 
...
when creating it at a script address as it was the case for PlutusV2 scripts. 
...
The [purple]#Datum# type is same as the [purple]#Redeemer# type a wrapper 
around the [purple]#BuiltinData# type. The datum allows information being 
...
The third option for a script purpose is rewarding that is used for 
withdrawing staking rewards. The rewarding constructor carries the 
...
or reference the validation script that will be used to decided whether 
the staking rewards can be withdrawn by the transaction. 

Both the [purple]#PubKeyHash# and [purple]#ScriptHash# data types are 
both wrappers around the [purple]#BuiltinByteString# data type. 

Other possibilities for the [purple]#ScriptInfo# data type cover the 
following script purposes: 

* Certifying - is for issuing certificates. 
* Voting and proposing - are used when government actions are involved. 

In the <<Plutus smart contracts>> section we will focus on the spending, 
minting and rewarding script purposes. 

*Decoding ScriptInfo in untyped form*
...
to the datum instead of the redeemer. 
...
The data is contained in a [purple]#Map# of credentials and lovelace. We will 
talk more about this in section <<Staking and Plinth>>. 

After that follows the transaction validity range, which is defined with the 
[purple]#POSIXTimeRange# data type. 
...
<<Cardano node layers>>). Once you have written on-chain code that works, 
...
Let’s look again at the Haskell definition of the [purple]#Value# type. 
This type is defined in the module `PlutusLedgerApi.V1.Value` and defines 
an amount of ada and/or native tokens. 
...
from the 5th week of the latest Plutus pioneer program. After that, we 
...
by transaction hashes, which are unique and output indices. The reason
...
Let’s look at an example of such minting policy. 
...
the [purple]#Mock# blockchain type – a wrapper around a list of UTXOs. Thus, 
...
cofigured. Instructions can be found in the `plinth-dev-env` GitHub. 
An up-to-date link to it, is available in the resources folder of the 
https://github.com/input-output-hk/mastering-cardano[mastering-cardano] 
...
topic, since these programs can manage significant amounts of funds. 
Estimates for the loss of funds due to various 
...
Hacks to blockchain smart contracts and bridges up to October 2023 alone
...
Because of this transactions are deterministic since their outcome is 
...
the user will not be charged any fees. Since transactions are deterministic 
in EUTXO, multiple transactions within a block that target the same validator 
can be processed. That works only if the transactions consume different 
inputs. In Ethereum this could be done in theory, while in practice a 
transaction targeting the same validator as another transaction inside 
a block could alter the global state such that second transaction would fail 
after the first one succeeds. This means that the state can change between 
when a user submits a transaction and when it is processed, which means the 
transaction can fail even after the user pays the transaction fee. 
One can read more on the topics of parallelism and concurrency in Cardano 
in the following 
https://www.essentialcardano.io/article/concurrency-and-all-that-cardano-smart-contracts-and-the-eutxo-model[IOG blog]
and https://academy.geniusyield.co/articles/what-is-cardano[GeniusYield Academy blog]. 
...
UTXO. We also note that in Cardano, it is possible that a user forces an invalid
...
attack very expensive. EUTXO’s determinism also means that users always
...
script validation in Cardano. According to this blog, in-determinism
...
with the help smart contracts that eliminate the need of a central entity 
...
liquidity providers get is often larger than the amount of impermanent loss.
...
Cardano's design makes also front-running invasion attacks harder to 
...
possible for an adversary that operates a stake pool inspect transactions 
...
at the time of writing) and also because of Cardano's design none of those 
nodes controls a significant amount of staked ada. You can read more about 
this topic in chapter <<Stake pools and stake pool operations>>. We also 
...
Because Cardano transactions are deterministic, their interaction with 
...
updates its state. This is known as a reentrance attack. Because this 
is possible in Ethereum it sometimes makes the effect of a transaction 
on the ledger hard to predict. Such an attack happened to the DAO smart 
contract https://www.coindesk.com/learn/understanding-the-dao-attack/[hacked in
...
run and all their inputs and outputs, so there's no way that an _untrusted_ 
...
explain the security advantages and best practices for them separately. 
...
using the https://atlas-app.io/[Atlas] application backend, the 
https://mlabs-haskell.github.io/clb-docs[Cardano ledger backend] 
emulator developed by MLabs and the QuickCheck library. 
Another tool one can use for property testing Plinth smart contracts
is the https://github.com/tweag/cooked-validators[Cooked validators]
library developed by Tweag. 
