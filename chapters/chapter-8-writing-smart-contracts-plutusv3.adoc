...
be packed into common lists and maps. Next, we show the high-level implementation 
...
First we look at the `cabal.project` file. 
...
in the `.cabal` file when compiling the project. Next we look at the `.cabal` file. 
...
In this section we will look into more detail at the [purple]#ScriptContext# data type. 
All data types contained in the script context will be presented as high-level Haskell data 
types. Let us recall the implementation of the script context data type. 
...
We state again if a transaction tries to spend multiple UTXOs at a script address, 
the spending script is run for every UTXO individually where the redeemer and the 
script information are individually assigned for every UTXO in that transaction,
in contrast to the transaction information, that is only one and can be accessed 
by every script that is triggered by a single transaction. 
