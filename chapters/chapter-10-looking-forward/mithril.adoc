...
_aggregator_, and _verifier_ (also known as the client) and
...
an adversary forging a valid multi-signature. The <<Mithril security (advanced)>> 
section provides details on the relationship between these parameters and protocol security.
...
which takes place within a predefined period, one epoch(((epoch))), which equals to five days on
...
as the _Mithril stake distribution_). Specifically, consensus must be
...
verification keys) is certified at the transition to a new
...
A malicious actor could create a fake stake
distribution relatively easily and use it to produce a valid multi-signature(((multi-signature))), which would
...
multi-signature(((multi-signature))), the distribution _must_ be verified as having been
...
documentation. The <<Additional resources>> section provides a link.
...
decentralization introduces complex issues and themes that are currently active
