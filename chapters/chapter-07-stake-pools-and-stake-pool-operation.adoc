...
Notice here that by the time we generated our first `operational certificate`, one kes-period passed. See the difference between `"qKesCurrentKesPeriod": 695` and `"qKesStartKesInterval": 694` !
...
Registering an address requires a deposit of two ada (2000000 lovelaces).
...
Since we only create a fake transaction in order to calculate transaction fees, we set the value withdrawn at zero.
...
* To add NTP servers, include their IP address (eg, 192.168.2.100) or DNS name (e.g., time.google.com) in the chrony.conf file.
...
Various techniques exist to scrape remote servers securely. See Server Hardening also.
