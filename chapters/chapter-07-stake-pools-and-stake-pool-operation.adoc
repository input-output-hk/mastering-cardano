...
As an open protocol, Cardano allows anyone to create and operate a stake pool using the free and open-source software(((free and open-source software))) (FOSS) – `cardano-node` and `cardano-cli`. However, it is essential for SPOs to possess the technical expertise required to operate a pool reliably and securely. While the requirements outlined in this section are not authoritative, they are generally recognized as critical for successful stake pool operation.
...
A strong understanding of and ability to navigate, utilize, and manage the Linux operating system is a must for any SPO. While cardano-node binaries are available for Linux, Mac, and Windows with each release, only Linux is supported in the block producer and relay roles. It is possible to build from source and operate cardano-node and cardano-cli on OpenBSD and potentially other Unix-based operating systems(((Unix-based operating systems))), but this is an advanced and separate topic.
...
==== Height battles
A height battle occurs when two blocks are presented at the same height and share the same parent hash, but have different slots.
...
When a battle or fork occurs, all nodes must reach consensus on which block to add to the chain and which one(s) to discard. In the Praos era (post-Vasil hard fork), two primary rules guide this decision:
...
But what if pool B creates a block in slot 15 and does _not_ have pool A’s block yet? This scenario leads to a height battle, as both pool A’s block in slot 10 and pool B’s block in slot 15 will share the same parent block. The contention is settled by comparing the block VRF(((block VRF))) values. The block with the lowest VRF value will be added to the chain and the other block will be discarded.
...
4.  Finally, in slot 15, Pool B’s block is distributed and all nodes need to pick between Pool B’s block and Pool C’s block when -if all pools were operating at full speed- Pool B’s block would have been the one added to the chain and Pool C’s block would have built on top of it.
...
This section focuses on the subset of `cardano-cli` commands most useful to Cardano SPOs(((stake pool))). It outlines the steps required to set up a stake pool from the very beginning, including:
...
Using the address generated above has one significant drawback. It can receive and send ada but has no staking rights associated with it, making it what we call an _enterprise address_(((address, enterprise))). To enable staking rights, we need to add a staking key to the address.
...
Your working directory should now contain seven files:
...
The node uses the VRF key pair to determine whether to attempt signing a block. At every slot, the node checks this condition.
----
...
Notice here that by the time we generated our first `operational certificate`, one kes-period had passed. See the difference between `"qKesCurrentKesPeriod": 695` and `"qKesStartKesInterval": 694` !
...
`--pool-cost`: the fixed cost(((fixed cost))) the pool will charge before calculating the margin fee. It cannot be set lower than 340 ada or 340,000,000 lovelaces at this time on mainnet.

`--pool-margin`: the percentage fee(((percentage fee))) taken by the pool on the remaining rewards after pool cost has been deducted from all block rewards. Its boundaries are zero (0 %) and one (100%). In this example, it is set at 2%
...
`--pool-pledge`: defines the minimum amount in lovelace that must collectively be present in the owner(s) account(s). If this requirement is not met, the pool will forfeit all rewards, both for delegators and the pool itself. To ensure flexibility or avoid risks in this example, it is set to zero.
...
* `--tx-in`: the UTXO that will be consumed in the format *`TxHash#TxIx`*. Nothing prevents you from consuming more UTXOs! Use additional `--tx-in` lines to do so.
...
You may adjust the `--tx-in-count`, `--tx-out-count`, and `--witness-count` values accordingly. Here, we consume one UTXO, have the change sent back in a single address, and will sign the transaction with three witnesses (a.k.a. secret keys).
...
*Note*: Transaction fees are deterministic. As such, they have a lower bound. However, users can always choose to pay more than the required amount. It is important to handle fee inputs with care.
...
Registering an address requires a deposit of two ada (2,000,000 lovelaces).
Registering a pool requires a deposit of 500 ada (500,000,000 lovelaces).
This means that the change to the `base.addr` will be : *_value = input - (502 deposit + transaction fee)_* Deregistration of an address or a pool will be accounted for by adding two ada or 500 ada, respectively, to the ada change value one must calculate to correctly balance a transaction! For example, to deregister simultaneously a base address and a pool: *_value = (input + 502 deposit) - transaction fee_*.
====

* `--invalid-hereafter`: The slot tip of the network plus some slots to give your transaction time to get accepted by the network. Here, 1,000 slots (seconds on mainnet) or ~ 15 minutes in the future, from the last query we made at `"slot": 90368116`.
* `--fee`: the exact value we calculated earlier.
* `--certificate-file`: adding a certificate to the transaction.


*Note:*  The order in which the certificates are declared will matter in the final transaction. You cannot delegate to a pool that does not exist yet, and you cannot register a pool with an owner’s address that is not registered yet. Hence, we register the `base.addr` (its staking part) first, then the pool, and finally delegate to it.
...
As the pool becomes saturated, it will eventually mint blocks and start accumulating ada rewards. These rewards, however, reside in the stake account associated with the base.addr and are not immediately available as UTXOs.
...
Since we only create a fake transaction to calculate transaction fees, we set the value withdrawn at zero.
...
This guide details all the necessary `cardano-cli` actions to become an autonomous SPO on Cardano. Hopefully, this document has clarified the numerous components involved.
...
Ouroboros is the protocol powering Cardano, symbolized by the serpent eternally consuming its own tail. Until https://eprint.iacr.org/2019/838.pdf[Ouroboros Chronos] is implemented, the ‘timeless’ Ouroboros relies on SPOs to help maintain accurate time.
...
* *Stratum 1*: suitable for systems that can deviate only within a few milliseconds of stratum zero
...
* On modern fiber connections near the timesync server, it is recommended to use a lower value for Chrony’s internal estimate of how fast or slow the computer clock runs relative to the timesync server. Setting maxupdateskew to 5.0 (down from the default 1,000 ppm) helps maintain higher precision in time synchronization.
...
* Rtsync can be set to enable the kernel to periodically synchronize the system clock with the real-time clock, typically every 11 minutes on Linux. If the connected timesync server does not provide leap second updates in advance, the leapsectz option (eg, leapsectz right/UTC) may be configured. In general, it is recommended to use Coordinated Universal Time (UTC) for server time, as this is the global standard and the format most GPS-based timesync servers report in.

With that out of the way in a fairly short time (pun intended), an SPO will be able to keep Cardano nodes collaborating well with the rest of the network in a timely manner!
...
*Note*: All examples listed here are suggestions for commands you can use on the Unix operating system. Please conduct your own research (DYOR) or refer to the command-related manual (RTFM) for verification.
...
* Change your server DNS to use known secure DNS (eg, 1.1.1.1 and 1.0.0.1)
...
It is beneficial for SPOs to use a virtual private network (VPN) (eg WireGuard) to access a server backend (such as Grafana or Prometheus) due to enhanced security and accessibility. A VPN establishes a secure and encrypted connection between the client device and the server backend, protecting transmitted data from potential eavesdropping or tampering.
...
These are typical targets to scrape for Prometheus for Cardano stake pools:
...
* The open source `node_exporter`, an open source Prometheus exporter for hardware and operating system metrics exposed by *NIX kernels for metrics on CPU, memory, disk usage, timesync and others.
...
Grafana is an open-source platform for monitoring and observability, specializing in data visualization and analytics. It allows operators to create dashboards, charts, and graphs to visualize and analyze metrics from various data sources. It is an ideal companion to display Prometheus metrics. SPOs generally opt to use Grafana alongside Prometheus for the following reasons:
...
https://www.zabbix.com/[Zabbix] is another integrated, all-in-one monitoring solution with out-of-the-box capabilities for monitoring diverse IT components and can be configured to monitor Cardano nodes. Prometheus and Grafana do offer more scalability, flexibility, and customization options, but Zabbix is a viable alternative.
...
RTView does render a web page dashboard and offers a less-complex but robust alternative to a well-configured Prometheus and Grafana monitoring setup.

==== Koios gLiveview

https://cardano-community.github.io/guild-operators/Scripts/gliveview/[Koios gLiveView] is a local monitoring tool to use besides remote monitoring tools like Prometheus and Grafana, Zabbix or RTView. This is especially useful for systemd deployments as it provides a terminal UI to monitor real-time node status for SPOs.
...
It is important for SPOs to be able to read and interpret logs from cardano-node to troubleshoot and find potential issues. It is best practice to investigate each missed block and determine what went wrong. Long-term luck should be near 100% and while block collisions within the same slot occur, they are uncommon. About 5% of all blocks result in slot battles, as the outcome of a slot battle is random, about 2.5% of blocks are expected to be lost over time.
...
** If the block was propagated within its slot time (one second) it was very likely a race condition and the other pool was awarded the block based on a lower VRF hash. Not much can be done about these conditions at this time besides working with the pool operators with misconfigured stake pools.
