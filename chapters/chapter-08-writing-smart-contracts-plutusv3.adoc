...
_Prof Philip Wadler(((Wadler, Philip
))), FRS_ +
...
))) (EVM) does for Ethereum. But where the EVM
...
elaboration of the lambda calculus(((lambda calculus
))) called System F(((System F
))), first defined by
...
was a business case for formally validating that theory. Input | Output (IO)(((IO
))) hired
James Chapman(((Chapman, James
))) to validate our variant of System F(((System F
))) in the Agda proof
...
certificates to ensure that phases of the compiler(((compiler
))) optimize code
...
is designed to represent the output of a compiler(((compiler
))), and no one should
...
to Manuel Chakravarty(((Chakravarty, Manuel
))), a contributor to both the design and
...
convenience). Marlowe, which is not a general-purpose language(((general-purpose language
))) but
...
IO. I'm particularly grateful to Charles Hoskinson(((Hoskinson, Charles
))) for the
...
smart contracts and integrating with Web3(((Web3
))) workflows through a TypeScript
SDK. The platform includes:

* *The Marlowe Runtime(((Marlowe Runtime
)))* supports all the necessary on- and
...
* *The Marlowe TypeScript SDK(((TS-SDK
)))* fully integrates Marlowe as a component
within a complete DApp, including contract creation and initialization,
as well as execution.

* *The Marlowe Playground(((Marlowe Playground
)))* provides the environment for learning Marlowe
through its Blockly(((Blockly
))) representation, as well as simulation and analysis
of contracts as they are developed.

* *End-to-end DApp examples* were developed in collaboration with Gimbalabs(((Gimbalabs
)))
with Catalyst Fund12(((Catalyst Fund12
))) support, and are designed to make access to the Marlowe
Platform as straightforward as possible.

* *Other components* support free-standing execution of Marlowe smart
contracts, including the Marlowe Runner(((Marlowe Runner
))) and the Marlowe CLI(((Marlowe CLI
))).

The platform simplifies development for the developer community and
provides key components, such as oracles(((oracles
))), used in many DApps. Individual
end users can also execute Marlowe contracts directly on the Cardano
blockchain, using the facilities that the Marlowe Runner(((Marlowe Runner
))) and the CLI provide.
...
Marlowe Language Community Interest Company (CIC)(((Marlowe Language Community Interest Company (CIC)
))), a non-profit
...
based on a novel pull-based model, where oracles(((oracles
...
the Marlowe TypeScript SDK(((TS-SDK
))), Aiken, and the Marlowe DApp starter kit(((DSK
))).
...
))) that run on the Cardano node
* Compiled languages that can be compiled to a native language
* Interpreted languages(((Interpreted languages
))) that are interpreted by a compiled language.

*Native languages(((Native languages
)))*

The Cardano node can only process native languages. Currently, there are
two available: simple scripts(((simple scripts
))) and Plutus.

The Shelley era(((Shelley era
))) introduced script addresses and
https://developers.cardano.org/docs/get-started/cardano-cli/simple-scripts/[simple scripts(((simple scripts
)))], also
known as native scripts(((native scripts
...
Allegra era extended simple scripts(((simple scripts
))) by adding conditional time-based
...
The Alonzo era(((Alonzo era
))) introduced the Plutus scripting language, also referred
to as Plutus Core(((Plutus Core
))). Plutus smart contracts validate transactions and because
...
)))). The compilation pipeline from a compiled language to
UPLC(((UPLC
))) is covered in section <<Plutus security>>.
...
contracts. A comparison of Plutus with Bitcoin Script(((Bitcoin Script
))) and Solidity(((Solidity
))) languages
is presented in section <<Plutus in comparison to Bitcoin Script(((Bitcoin Script
))) and Solidity(((Solidity
)))>>.
...
* https://github.com/mlabs-haskell/purus[Purus]: a PureScript to Plutus Core(((Plutus Core
))) compiler(((compiler
)))
...
One can read more about languages that compile to UPLC(((UPLC
))) at the official
...
developer to understand other compiled languages(((compiled languages
))) that use different syntax.
...
* The https://www.emurgo.io/press-news/the-programming-languages-behind-cardano-on-chain-code/[Emurgo(((Emurgo
)))]
blog about the programming languages behind Cardano on-chain code
* The https://www.essentialcardano.io/article/programming-languages[Essential Cardano(((Essential Cardano
)))]
...
https://mlabs-haskell.github.io/uplc-benchmark/comparison.html[UPLC(((UPLC
))) benchmark]
...
https://www.geniusyield.co/[GeniusYield(((GeniusYield
)))])
...
* https://developers.cardano.org/showcase[Cardano developers showcase(((Cardano developers showcase
)))]
...
* Bonds(((Bonds
))), forwards, options, futures(((futures
))), swaps, etc
...
https://github.com/IntersectMBO/cardano-cli/tree/main[Cardano CLI(((Cardano CLI
)))], or
...
https://no-witness-labs.github.io/evolution-sdk/[Evolution-SDK(((Evolution-SDK
)))]
project, which was temporary called Lucid Evolution and
was funded by
https://projectcatalyst.io/funds/11/cardano-open-developers/anastasia-labs-lucid-evolution-redefining-off-chain-transactions-in-cardano[Catalyst Fund11(((Catalyst Fund11
)))].
...
The World Mobile network(((World Mobile network
))) consists of three layers of nodes, each with
...
the https://worldmobiletoken.com/[World Mobile Token(((WMT
)))] (WMT) as the
...
2023. These NFTs(((NFTs
))) provide monthly rewards; each month, Earth node owners
...
* *Privacy*: user information is stored using private/public encryption(((private/public encryption
)))
provided by Cardano
* *Immutability*: Cardano's EUTXO model ensures transaction immutability
and restricts spending to only unspent transaction outputs(((transaction outputs
))), making the
...
Shelley era(((Shelley era
))), a Cardano address consists of two parts: the _payment_ part
...
))), containing the same owner information. The payment part(((payment part
))),
...
key or a Plutus script. If it contains a public key hash(((public key hash
))), UTXOs can only
be spent if the transaction is signed with the corresponding private
(signing) key. If it contains a script hash(((script hash
))), the script executes during
...
defined with a public key hash(((public key hash
))), the corresponding private key(((private key
))) owner can
spend the rewards. If defined with a script hash(((script hash
))), the script determines
the conditions under which staking rewards can be spent.

Cardano Shelley addresses(((Shelley addresses
...
))). Rewards are reflected in accounts, and UTXOs are created
only when rewards are withdrawn.  +
 +
The Shelley era(((Shelley era
))) continued to support Byron-era(((Byron-era
))) _bootstrap addresses_ and
_script addresses_. The
https://docs.cardano.org/learn/cardano-addresses/[Cardano addresses(((Cardano addresses
)))]
...
characters. https://en.bitcoin.it/wiki/Bech32(((Bech32
)))[Bech32(((Bech32
)))] and
https://bitcoinwiki.org/wiki/base58[Base58(((Base58
)))] are encodings used in
...
the underlying byte sequences. Shelley addresses(((Shelley addresses
))), which include staking
addresses, use Bech32(((Bech32
))) encoding without a character length limit. In
contrast, Byron addresses(((Byron addresses
))) are encoded in Base58(((Base58
))), allowing for easy
...
In Cardano addresses(((Cardano addresses
))), the sequence of bytes (decoded from _Bech32_ or
...
))), including the addresses used for staking. You can see
...
https://cips.cardano.org/cips/cip19/#binaryformat[Cardano Improvement Proposal 19 (CIP-19(((CIP-19
))))], which defines the technical details of Cardano
addresses.
...
real-world data providers through oracles(((oracles
))), enabling contract
...
Marlowe to the Marlowe Foundation(((Marlowe Foundation
...
The https://playground.marlowe-lang.org/[Marlowe Playground(((Marlowe Playground
)))] is the
...
includes Blockly(((Blockly
))) – an editor for visual programming. Together, these
...
command-line expertise. With Marlowe Runner(((Marlowe Runner
...
or https://eternl.io/[Eternl(((Eternl
)))]. Contracts can be uploaded to the Runner as
...
contract uses role tokens(((role tokens
))), the funds cannot be retrieved from the
...
tutorial] about using Marlowe Runner(((Marlowe Runner
))) can be found on the IO YouTube channel.

https://github.com/marlowe-lang/marlowe-scan[Marlowe Scan(((Marlowe Scan
)))] is a tool
...
components. Marlowe Runtime(((Marlowe Runtime
))) backend services are off-chain components
...
* Using Marlowe Runtime(((Marlowe Runtime
))) web REST API
...
scripts, and Plutus scripts, the Marlowe CLI(((Marlowe CLI
))) tool provides the
...
develop custom tool sets. This allows them to wrap the Marlowe CLI(((Marlowe CLI
))) tool
...
and Marlowe Runtime(((Marlowe Runtime
...
lessons demonstrating the use of the Marlowe CLI(((Marlowe CLI
))) and Marlowe Runtime(((Marlowe Runtime
)))
tools through concrete examples. It can be used with Demeter Run, a
Docker(((Docker
))) deployment of Marlowe Runtime(((Marlowe Runtime
))), and Nix(((Nix
))) to run the Jupyter
...
libraries, available as npm(((npm
))) packages, designed to support DApp
...
the SDK in a Node.js(((Node.js
))) environment.

The https://demeter.run/[Demeter.Run(((Demeter.Run
...
about the platform in section <<Setting up a Plinth development environment(((Plinth development environment
)))>>.
...
Below is the Marlowe Runtime(((Marlowe Runtime
))) architecture:
...
The Marlowe Runtime(((Marlowe Runtime
))) backend consists of a chain-indexing and query
...
https://cips.cardano.org/cips/cip30/[CIP-30(((CIP-30
))) light wallet] for
...
Please note that the Marlowe Runtime(((Marlowe Runtime
))) architecture may evolve. Refer to
...
this case, role tokens(((role tokens
))) are not created. In a Marlowe contract, internal
...
Blockly(((Blockly
))) in the Marlowe Playground(((Marlowe Playground
))) in the following
https://youtu.be/fldaBHmYfqk[Plutus Pioneer program(((Plutus Pioneer program
)))] video.
...
Next is a flowchart generated with the Marlowe Runner(((Marlowe Runner
))) that indicates
...
Below, you can see the image of the contract implemented in the Blockly(((Blockly
)))
tool:
...
in POSIX time(((POSIX time
))) (measured as milliseconds after UNIX epoch –
...
The next image shows the contract implemented using the Blockly(((Blockly
))) tool:
...
token trades, and ownership of role tokens(((role tokens
))) can also be traded for
...
))), such as Aiken, for example. This section focuses on
...
participants' Cardano addresses(((Cardano addresses
))) are unknown at deployment. When using
open roles, the Marlowe contract sends role tokens(((role tokens
))) to the
...
participants, while Marlowe Runtime(((Marlowe Runtime
))) manages the rest. The
...
The Marlowe Foundation(((Marlowe Foundation
))) will continue supporting Marlowe by consolidating
and extending it for Cardano DApp builders. This includes providing
oracles(((oracles
))) and micropayments(((micropayments
))), optimizing execution, and supporting runtime
monetization. Marlowe offers a lower barrier to entry for DApp
development. The new Marlowe DApp starter kit(((DSK
))) (DSK), which will be
maintained by the Marlowe Foundation(((Marlowe Foundation
))), will highlight these advancements
and streamline onboarding for small and medium-sized businesses,
developers, infrastructure providers, and stake pool operators(((SPOs
))).
...
stack, transforming the Marlowe Runtime(((Marlowe Runtime
))), protocols, and contracts into
...
The Marlowe DApp starter kit(((DSK
))) (DSK) will consolidate the documentation,
...
the complexities of the UTXO model(((UTXO model
))) through a user-friendly yet robust
...
and the Marlowe Runtime(((Marlowe Runtime
))), which integrates with familiar REST APIs, make
...
the initial onboarding process to a single npm(((npm
))) install command, a surge
...
oracles(((oracles
))) and micro-payments in the Marlowe product will benefit
...
inclusion of features like micropayments(((micropayments
))) and oracles(((oracles
))) will also attract
...
The _Plutus smart contracts_ section presents the rules of the Plutus language by demonstrating how to write and interact with Plinth smart contracts (formerly known as PlutusTx). Plinth is a high-level smart contract language embedded in Haskell that compiles to Untyped Plutus Core(((Plutus Core
))) (UPLC(((UPLC
)))).
Because these rules also apply to other compiled languages(((compiled languages
))) such as Aiken, Plu-Ts, and OpShin, learning the basics of Plinth helps developers read and write smart contracts in other languages that target Plutus.
...
Aiken learning resources. Plinth, as a high assurance language embedded in Haskell,
...
code. The advantages of the Haskell programming language are presented
...
PlutusV3, which became available after the Chang hard fork(((Chang hard fork
))). From the
...
advanced Plutus cryptographic primitives(((cryptographic primitives
))), other new primitives, and
...
* Plutus in comparison to Bitcoin Script(((Bitcoin Script
))) and Solidity(((Solidity
)))
...
branch of the Plutus Pioneer program(((Plutus Pioneer program
))) (PPP) GitHub repository. This
...
* Gimbalabs(((Gimbalabs
))) https://plutuspbl.io/[Plutus project-based learning] –
...
online book written by John Greene(((Greene, John
))) that is available under an open-source license.
...
))) technology, briefly introduced in section
...
A Bitcoin Script(((Bitcoin Script
))) is a simple stack-based smart contract language, whose
most complex control structure is a conditional. It is written in a
Forth-like non-Turing-complete(((Turing-complete
))) language and is essentially linear, which
...
[arabic]
. The Bitcoin Script(((Bitcoin Script
))) language constrains programs to be of a limited
...
))) are also very limited (for example, the division
operation was originally included but was subsequently disabled).
. The computational context available to a Bitcoin Script(((Bitcoin Script
))) program is
...
groups (eg, _Massimo Bartoletti and Roberto Zunino. 2018. BitML: A
Calculus for Bitcoin Smart Contract_).

Ethereum provides a Turing-complete(((Turing-complete
))) language for the Ethereum virtual
machine (EVM), which is the runtime environment for transaction
execution in Ethereum. It also provides Solidity(((Solidity
))), a custom higher-level
programming language that compiles into EVM code. Solidity(((Solidity
))) is an
...
contract transaction on Ethereum. A Solidity(((Solidity
))) smart contract is able to
...
that a user has to pay gas fees(((gas fees
))) for a transaction that interacts with a
...
a Solidity(((Solidity
))) smart contract. Plutus scripts cannot see the whole state of the
...
))) or an off-chain code library to check whether a transaction will
...
conditions are not met, the transaction will fail without charging the user any fees, unlike in Ethereum, where users must pay gas fees(((gas fees
))) for transactions
...
or more precisely to Untyped Plutus Core(((Plutus Core
))) (UPLC(((UPLC
)))), which the Cardano nodes execute.
Plinth provides a compiler(((compiler
))) plugin for the Glasgow Haskell Compiler (GHC), which
...
Plinth compiler(((compiler
))) (previously called PlutusTx), enabling developers to
...
language written in Rust, which directly compiles to UPLC(((UPLC
...
and the https://cardanofoundation.org/[Cardano Foundation(((Cardano Foundation
)))].
...
The https://plutus.cardano.intersectmbo.org/docs/working-with-scripts/optimizing-scripts-with-asData[Plinth user guide(((Plinth user guide
)))]
...
* Run a Nix(((Nix
))) shell that contains all the dependencies
* Run a local Docker(((Docker
))) container inside VSCode
...
instructions for the latest stable Plinth development environment(((Plinth development environment
))). The smart
contract code presented throughout section <<Plutus smart contracts>> uses features
introduced in the Chang hard fork(((Chang hard fork
))), which enabled PlutusV3. The development
...
with a Nix(((Nix
))) shell that was set up with Nix(((Nix
))) instructions provided in the _plinth-dev-env_
repository. Nix(((Nix
))) version `2.25.3` was used and a Nix(((Nix
))) shell provided by
...
The https://demeter.run/[Demeter.Run(((Demeter.Run
)))] online platform offers various
...
https://aiken-lang.github.io/stdlib/[standard library(((standard library
)))] for the Aiken
language.

Plinth data types cannot be explored in the standard GHCi REPL because the
GHC compiler(((compiler
))) pipeline first compiles Plinth code to an intermediate
language called Haskell Core. The Plutus compiler(((compiler
))) then takes this
...
The redeemer and the script information(((script information
))) are individually assigned for every
...
The long list of language pragmas, GHC compiler(((compiler
))) options, and import statements
...
also cover various GHC compiler(((compiler
))) options. We name our module `Week02.Validators`.
...
after the function definition, the GHC compiler(((compiler
...
represented as a `CBORHEX(((CBORHEX
)))`. The https://en.wikipedia.org/wiki/CBOR[Concise Binary
Object Representation] (`CBOR(((CBOR
)))`) is a binary data serialization format loosely based
...
Since our contract is simple, the `CBORHEX(((CBORHEX
)))` value is short. For more complex
...
----
{- ------------------------------------------------------------------------------ -}
{- ----------------------- 42 validator untyped large CBOR(((CBOR
))) ---------------------- -}
...
The reason why we stated _"large CBOR(((CBOR
)))"_ in the comment above the code is that
converting the script context into a typed form is a costly operation and produces
a large CBOR(((CBOR
))). Next, let us look at the same validator in untyped form, where we decode
the script by keeping it in the form of the [purple]#BuiltinData# type. At the end of
this section, we will compare `CBORHEX(((CBORHEX
)))` lengths for all 42 validators presented in
this section.

[source,haskell]
----
{- ------------------------------------------------------------------------------ -}
{- ----------------------- 42 validator untyped small CBOR(((CBOR
))) ---------------------- -}
...
the `CBORHEX(((CBORHEX
)))` lengths decrease by around 10%. The table below shows compiled
...
performance advantage. The largest `CBORHEX(((CBORHEX
)))` is the one for the untyped
...
interface that works directly with the Plutus Core(((Plutus Core
))) [purple]#Data# type under the hood. Because
...
https://plutus.cardano.intersectmbo.org/docs/working-with-scripts/optimizing-scripts-with-asData[Plinth user guide(((Plinth user guide
)))]
...
))) definitions for two validators
that we have chosen.
...
mk42ValSmall =
  MkValidatorBlueprint
    { validatorTitle = "42 Validator untyped - small CBOR(((CBOR
)))"
...
In `.cabal`, we define our compiler(((compiler
))) options, the library where our validator
...
To reiterate: if a transaction attempts to spend multiple UTXOs at a script address, the spending script is executed separately for each UTXO. The redeemer and script information(((script information
))) are assigned individually to each UTXO, whereas the transaction information is shared across all scripts triggered by that transaction.

In the previous code examples, the validation logic did not use the transaction and script information(((script information
))). However, in most validator scripts, they are
used. First we look at the script information(((script information
))) data type.
...
The script information(((script information
))) contains information about the script's purpose.
...
))) contains the transaction ID(((transaction ID
))), which is an SHA-256
...
0 and increasing by 1. Because transaction hashes (IDs) are unique, the transaction hash(((transaction hash
))) defines a UTXO, and the output index is also unique and can exist only once.
The reason transaction hashes are unique is because a new transaction hash(((transaction hash
))) includes
...
This data type either contains a public key hash(((public key hash
))) or a script hash(((script hash
))). In
the case of a public key hash(((public key hash
))), the owner of the private key(((private key
))) corresponding
to the public key needs to sign the transaction to be able to spend the
staking rewards. In the case of a script hash(((script hash
...
*Decoding the script information(((script information
))) in untyped form*
...
and reference inputs(((reference inputs
))). Reference inputs are inputs accessible by the script
...
<<Off-chain code with MeshJS>>. Another advantage of reference inputs(((reference inputs
))) is
...
Transaction inputs and reference inputs(((reference inputs
))) are lists of type transaction input
...
two parts: the payment part(((payment part
))) and the optional staking part. The payment part(((payment part
))),
defined with the [purple]#Credential# data type, contains either a public
key hash or a script hash(((script hash
))) as we have explained when talking about rewarding.
...
A script hash(((script hash
))) is also a wrapper around a [purple]#BuiltinByteString#.

Next in the [purple]#TxInfo# data type are the transaction outputs(((transaction outputs
))) which the
...
and the _Spending_ constructor holds only a transaction output reference(((transaction output reference
))) without a maybe datum.
The keys for the datums are datum hashes that map to actual datums.

Next comes the transaction ID(((transaction ID
))) of type [purple]#TxId# which represents the
...
the transaction information are the current treasury amount(((treasury amount
))) and treasury
...
beneficiaries public key hash(((public key hash
))) and the deadline in form of [purple]#POSIXTime#
...
[purple]#VestingDatum#. It holds the beneficiary's public key hash(((public key hash
))) and the
...
the beneficiary's public key hash(((public key hash
))) from the vesting parameter. Apart from that,
...
After the Alonzo era(((Alonzo era
))), when smart contracts became available on Cardano,
...
https://github.com/IntersectMBO/cardano-cli[Cardano CLI(((Cardano CLI
)))]. You can
find examples and read more about the Cardano CLI(((Cardano CLI
))) at the
...
developed by https://www.geniusyield.co/[GeniusYield(((GeniusYield
...
https://docs.npmjs.com/downloading-and-installing-node-js-and-npm[npm(((npm)))]
...
https://www.npmjs.com/package/@meshsdk/core[npm(((npm
...
If no `package.json` file is present `npm(((npm
)))` will create one and install
the latest version of MeshSDK packages. For more information on how to
install npm(((npm
))) packages locally or globally and manage the `package.json`
...
a package.json file] npm(((npm
))) documentation pages. The `deno` tool can be
...
the `@meshsdk/common` libraries. Then we import the seed phrase(((seed phrase
))) of our
...
class. Next we read out some wallet information and define our vesting script(((vesting script
))).
...
First, we define the wallet address and the public key hash(((public key hash
))) that
belongs to that address. We will set the beneficiary to our own
public key hash(((public key hash
))) so we can claim the funds back ourself. Then we define
...
define our vesting script(((vesting script
))) and compute the address of this script. We
note that we have to apply the function [blue]#applyCborEncoding# that
takes the raw script `CBORHEX(((CBORHEX
)))` from the blueprint and formats it in
...
The [blue]#sendFunds# function in the end then returns the transaction hash(((transaction hash
)))
...
and a transaction hash(((transaction hash
))) both of type [purple]#string#. It then checks if,
...
claiming funds from the vesting script(((vesting script
))) address.

The [blue]#claimFunds# function takes in a transaction hash(((transaction hash
))), which should
...
with the public key hash(((public key hash
))) we have set in the datum of the vested UTXO.

Once we have created our transaction we again have to build it,
sign it and submit it. Then we return the transaction hash(((transaction hash
))). At the
...
we call with the transaction hash(((transaction hash
))) the previous function call returns
...
the first command again, copy the logged transaction hash(((transaction hash
))) to the second
...
As before, we define our wallet address, public key hash(((public key hash
))) and
...
public key hash(((public key hash
))) and the deadline we have defined. Next we define our
parameterized vesting script(((vesting script
))). We again shorten the `CBORHEX(((CBORHEX
)))` compiled
...
up the UTXO at our wallet. Then we define a transaction builder(((transaction builder
))) that we
will use to build the transaction. After that we define our transaction.
We first specify that we want to deploy our vesting script(((vesting script
))) at the burn
...
and return the transaction hash(((transaction hash
))). Next, we can define our functions
...
as in the previous code. It takes in the transaction hash(((transaction hash
))) of our
UTXO we have vested at the script, and also the transaction
hash of the UTXO that contains our deployed vesting script(((vesting script
))). In the body
...
and submit the transaction and return the transaction hash(((transaction hash
))).

At the end, we define the function calls by logging their return values.
We send 3 ada to the parameterized vesting script(((vesting script
))). Then we deploy the
...
Because Cardano uses the hard fork combinator(((HFC
))) technology, all Plutus
...
parameterized, accepting an additional public key hash(((public key hash
))) as a parameter.
...
is parameterized with a public key hash(((public key hash
))) and the validation checkes if
...
read out our wallet address and the public key hash(((public key hash
))). And then we
define our minting policy, where we apply the public key hash(((public key hash
))) as
...
))). Now we can make our function call and mint the tokens.
...
Plutus also allows the minting of non-fungible tokens(((NFTs
))) (NFTs(((NFTs
)))), which are
...
minting policy – namely, a UTXO transaction hash(((transaction hash
))) and ID – and have the policy
...
output reference is made up of two types, which are the transaction hash(((transaction hash
)))
and output index. Before we compile our script in untyped form, we can change
it such that it takes in the transaction hash(((transaction hash
))) and output index as two separate
...
// Define address and public key hash(((public key hash
))) of it
const walletAddress: string = await wallet.getChangeAddress();
...
))). We again shorted the compiled code and parameterize
the code with the transaction hash(((transaction hash
))) and output index of the UTXO we
...
the transaction hash(((transaction hash
))). When we call the minting function,
...
https://cips.cardano.org/cip/CIP-68[CIP-68(((CIP-68)))] 
...
* BLS12-381(((BLS12-381
))) is about elliptic curve pairing and including 17 primitives
...
))) is a cryptographic hash function(((cryptographic hash function
))) that enables hashing of
public keys and scripts within Plutus, facilitating various interesting
applications
* Keccak-256(((Keccak-256
))) is another cryptographic hash function(((cryptographic hash function
))) that supports
Ethereum signature verification within Plutus scripts.

These cryptographic primitives(((cryptographic primitives
))) bring new possibilities, unlock better
...
for encoding data types in Plutus Core(((Plutus Core
))) is the Scott approach(((Scott approach
))). With the
introduction of PlutusV3, sums of products allow for encoding data types
...
script efficiency and optimize code generation for Plutus Core(((Plutus Core
))) compilers.
...
to https://cips.cardano.org/cip/CIP-0085[CIP-85(((CIP-85
)))]. Sums of products do
...
https://www.emurgo.io/press-news/why-your-web3-dapp-needs-a-smart-contract-audit/[Emurgo(((Emurgo
)))]).
...
taken from the https://rekt.news/leaderboard/[Rekt leaderboard(((Rekt leaderboard
)))]). Out
...
an AMM DEX where the deposited token pair is ETH and USDT (Tether(((Tether
)))),
...
During this time,
https://www.coindesk.com/learn/crypto-arbitrage-trading-how-to-make-low-risk-gains/#:~:text=In%20its%20simplest%20form%2C%20crypto,involves%20little%20or%20no%20risks[arbitrage traders(((arbitrage traders
)))]
...
as Ethereum does with https://en.wikipedia.org/wiki/Ethereum#ERC20[ERC-20 tokens(((ERC-20 tokens
)))].
...
This https://www.emurgo.io/press-news/why-crypto-projects-built-on-cardano-are-more-secure/[Emurgo(((Emurgo
)))
...
Custom Tokens in the Extended UTXO(((EUTXO
))) Model] scientific paper.
...
Further learning resources about the EUTXO model in addition to section
<<The EUTXO model>> can be found at:

* https://ucarecdn.com/6d3813f2-6886-4c61-833f-e78ba5f887d7/EUTXOhandbook_for_EC.pdf[The EUTXO Handbook(((The EUTXO Handbook
)))]
...
Cardano offers Plinth, previously called PlutusTx, which is a Turing-complete(((Turing-complete
)))
...
))) (known simply as _Plutus_), which is the scripting
...
organisation (MBO(((MBO
)))) and is called https://github.com/pragma-org/amaru[Amaru(((Amaru
)))].
...
https://en.wikipedia.org/wiki/Lambda_calculus[lambda calculus(((lambda calculus
)))],
specifically, https://en.wikipedia.org/wiki/System_F[System F(((System F
)))]. Lambda
...
This https://www.mlabs.city/blog/testing-dapps-on-cardano-with-clb-emulator[MLabs(((MLabs
))) blog]
...
https://cips.cardano.org/cip/CIP-52(((CIP-52
)))[CIP-0052],
which defines guidelines and best practices for auditing DApps built on
Cardano. Furthermore, https://cips.cardano.org/cip/CIP-57(((CIP-57
)))[CIP-0057]
...
methodology. This
https://www.tweag.io/blog/2023-05-11-audit-smart-contract/[Tweag(((Tweag
)))
...
This https://library.mlabs.city/common-plutus-security-vulnerabilities[MLabs(((MLabs
)))
...
* https://github.com/tweag/tweag-audit-reports[Tweag(((Tweag
))) audit reports]
...
* Use the https://playground.marlowe-lang.org/#/[Marlowe Playground(((Marlowe Playground
)))] to
