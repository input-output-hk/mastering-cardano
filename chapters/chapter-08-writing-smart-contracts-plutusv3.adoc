...
Smart contracts(((Smart contracts))) for the Cardano blockchain run in a language called
...
smart contracts(((smart contracts))) and integrating with Web3(((Web3)))
...
(CIC)(((Marlowe Language Community Interest Company (CIC)))), a non-profit
...
scripts(((simple scripts)))], also known as native scripts(((native scripts))). 
...
Plutus Core(((Plutus Core))) (UPLC(((UPLC)))). The compilation pipeline from a
compiled language to UPLC(((UPLC))) is covered in section <<Plutus security>>.
Plutus is a simple, functional language that enables general-purpose
Turing-complete(((Turing-complete))) smart contracts(((smart contracts)))
on Cardano. It implements Cardano's extended UTXO
(EUTXO(((EUTXO)))) model, which is as powerful in expressing smart contract logic
...
* The https://www.essentialcardano.io/article/programming-languages[Essential Cardano(((Essential Cardano)))] blog about programming languages.
...
https://mlabs-haskell.github.io/uplc-benchmark/comparison.html[UPLC(((UPLC))) benchmark]
...
showcase(((Cardano developers showcase)))] page: tags projects by groups
...
https://marlowe-lang.org/[Marlowe(((Marlowe)))], initially developed by IO, is an
...
https://github.com/IntersectMBO/cardano-cli/tree/main[Cardano CLI(((Cardano CLI)))], or
...
contracts(((smart contracts))), the company provides a sharing economy model to deliver
...
*World Mobile token and Earth node non-fungible tokens (NFTs(((NFTs))))*

Utilizing Cardano's native token capabilities, World Mobile introduced
the https://worldmobiletoken.com/[World Mobile Token(((WMT)))] (WMT) as the
...
image::sc_cardano_address.png[title="Cardano address"]
...
characters. https://en.bitcoin.it/wiki/Bech32[Bech32(((Bech32)))] and
https://bitcoinwiki.org/wiki/base58[Base58(((Base58)))] are encodings used in
...
image::sc_address_types.png[title="Address types, source: https://cips.cardano.org/cip/CIP-19[CIP-19]"]
...
image::sc_address_structure.png[title="Address structure, source: https://cips.cardano.org/cip/CIP-19[CIP-19]"]
...
Marlowe(((Marlowe))) is a domain-specific language (DSL(((DSL)))) for building
...
)))* – a simulator that allows testing Marlowe smart contracts
...
image::sc_marlowe_tools.png[title="Marlowe tools, source: https://docs.marlowe-lang.org/docs/developer-tools/overview[Marlowe docs]"]
...
image::sc_marlowe_runtime_architecture.png[title="Marlowe runtime architecture, source: https://github.com/marlowe-lang/marlowe-cardano/blob/main/marlowe-runtime/doc/ReadMe.md#architecture[Marlowe GitHub]"]
...
))) are not created. In a Marlowe contract(((Marlowe contract))), internal
...
image::sc_marlowe_contract_tree.png[title="Marlowe contract tree"]

Below, you can see the image of the contract implemented in the Blockly(((Blockly
)))
tool:

image::sc_marlowe_contract_choice.png[title="Marlowe choice contract"]
...
image::sc_marlowe_contract_escrow.png[title="Marlowe escrow contract"]
...
image::sc_marlowe_2025_plan.png[title="Marlowe 2025 plan"]
...
The _Plutus smart contracts(((smart contracts)))_ section presents the rules
of the Plutus language by demonstrating how to write and interact with Plinth(((Plinth)))
smart contracts (formerly known as PlutusTx(((PlutusTx)))). Plinth is a high-level smart
contract language embedded in Haskell that compiles to Untyped Plutus Core(((Plutus Core)))
(UPLC(((UPLC)))). Because these rules also apply to other compiled
...
understanding of the extended UTXO (EUTXO(((EUTXO)))) accounting model that is presented
...
Plinth(((Plinth))) enables developers to write and compile their on-chain code to Plutus,
...
https://aiken-lang.org/[Aiken(((Aiken)))] is another popular Cardano smart
contract language written in Rust, which directly compiles to UPLC(((UPLC))).
This process is illustrated in the diagram below:

image::sc_aiken_compilation.png[title="Aiken code transformation, source: https://cardanofoundation.org/blog/aiken-the-future-of-smart-contracts[CF blog]"]
...
This section explores the https://meshjs.dev/[MeshJS(((MeshJS)))] tool, developed by
...
submit transactions. We use https://blockfrost.io/[BlockFrost(((BlockFrost)))]. For the
...
image::sc_loss_of_funds.png[title="Loss of funds, source: https://www.cyfrin.io/blog/competitive-vs-private-audits-comparison[Cyfrin blog]"]
...
snapshot) is held by honest participants. The extended UTXO (EUTXO(((EUTXO))))
...
The Glasgow Haskell Compiler (GHC(((GHC)))), that is the most well-established Haskell
compiler(((compiler
))), is used to compile Plinth code. On-chain code is compiled into
Untyped Plutus Core(((Plutus Core
))) (known simply as Plutus(((Plutus)))), which is the scripting
...
image::sc_plutus_compilation.png[title="Plutus compilation pipeline"]
...
languages like https://aiken-lang.org/[Aiken(((Aiken)))]. A comparison of Aiken, which
