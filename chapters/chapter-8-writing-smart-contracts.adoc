...
This chapter focuses on the data types defined for Plutus V2 scripts. The
...
[purple]#Data# type. Those functions only work in off-chain code but not 
in on-chain code. The [purple]#Data# type has its constructor exposed, as
...
policy receives its own redeemer and script context as input. All
