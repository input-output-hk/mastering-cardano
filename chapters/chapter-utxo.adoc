...
The first phase consists of "cheap", quick checks. They do not incur a fee, even if they fail. Those checks include the "indeterministic" aspects of validation, things that cannot be checked before submitting the transaction.
...
Instead of just searching for a payment output to Alice with the correct price, the script can instead search for such an output that _also_ has the _UTxO reference_ of the NFT output in its datum.
