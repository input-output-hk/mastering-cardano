...
the EUTxO model introduces addresses that contain hashed _scripts_, written in the scripting language
_Plutus Core_.
...
It turns out that this design hits a sweet spot regarding expressiveness and security:
    ...
    * Ethereum smart contracts are very powerful and flexible, but also extremely hard to get right.
      Their context is the whole blockchain, which makes it difficult to predict what will happen when they get executed.
...
on Ethereum, but also easy enough to make it much easier to predict the effect of a given transaction.
