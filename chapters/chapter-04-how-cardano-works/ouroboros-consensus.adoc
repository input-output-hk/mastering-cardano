...
_temporary forks_ may occur, i.e. for a moment in time there will be two versions of what could eventually be in the canonincal blockchain.
...
- There is no incentive to form mining pools (teams), since the chance of being selected as the next slot leader is proportional to the amount of stake — i.e., one's _investment_ in the network or the trust of other users that delegate their stake to SPOs
...
The name _Ouroboros_,the ancient symbol of a snake eating its own tail, reflects how each epoch feeds into the next.
