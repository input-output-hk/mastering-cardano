...
block production, read sections <<Reaching consensus using proof of stake>>,
and <<about-the-ouroboros-protocol, About the Ouroboros protocol>>.
...
proof-of-stake and Ouroboros sections.
...
Foundation. After CIP-1694 will be fully implemented, genesis keys won’t be used
...
sometimes also referred to as Plutus Core or Untyped Plutus Core (UPLC).
It provides Turing-complete
smart-contract capabilities to Cardano and can be processed by Cardano
nodes. Plutus is based on untyped lambda calculus and acts as low-level
...
Plutus is explained in section <<Plutus security>>.
