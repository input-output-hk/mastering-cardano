...
The goal of the incentives mechanism is to lead to a configuration where a solid majority of stake is delegated to a number of k(((k (parameter)))) stake pools of roughly equal size, whose operators' nodes are online when needed and provide additional infrastructure (like relay nodes)(((relay nodes))).
