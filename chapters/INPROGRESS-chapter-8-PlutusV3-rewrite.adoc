...
Section <<PlutusV3 features>> shortly explains some of the PlutusV3
features that are new in comparison to PlutusV2 and their advantages. 
...
changes. The 4th iteration presents PlutusV2 examples. The GitHub repository 
contains also YouTube links for lectures that explain the code examples. 
The IO education team also translated the Plinth code examples from the 4th 
...
in the datum that is included in the transaction context. Finally, the 
...
contained in the transaction context. A Plutus smart contract can use all 
these three pieces of information to decide whether it is ok to consume an 
...
or tpye class that you are queriying. This is because implementations 
if types, functions and type clases can change with a new Plutus version 
and the haddock documentation keeps track for all of them. 
Once you have located the desired item, click the _Source_ icon next to its
