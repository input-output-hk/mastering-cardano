...
Also, a side effect of EUTXO is that bad actors which would try to 
...
mempol independent of the transaction fees assiciated with them. This 
is not the case in all blockchains where a node might favor transactions 
...
same final ledger state at the end. And it cannot be done for dependent 
transactions because transaction 2 inputs won't exist unles transaction 1 
outputs are generated first. 
