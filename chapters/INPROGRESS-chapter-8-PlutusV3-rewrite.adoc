...
bellongs to that address. Then we define the deadline, which sets 
the time after which the funds can be claimed from the script address. 
We convert it to `POSIX` time in milliseconds. After we have define 
our data that we will attach in the datum we can define our vesting 
script and compute the address of this script. The compiled code 
is shortened in our code snippet. When running this code the actual 
`CBORHEX` of the validator has to be provided that can be found in the 
`blueprint.json` file at the 
...
The [blue]#sendFunds# takes an amount of lovelace provded as a string, 
...
Mesh source code states that setting the network is mainly used to know the 
...
representing the following custom data type we use in our validator code:



After that we set the change address to our 
address which means that the lovelace change amout of the UTXOs we will 
spend will go back to our wallet. 
