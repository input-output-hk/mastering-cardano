...
works directly with the [purple]#Data# type under the hood. Now data types can 
be thin wrappers over the [purple]#BuiltinData# type which allows to retain the 
user friendliness of the datatype version while also avoiding the upfront cost 
of decoding the [purple]#BuiltinData# into sums of products. We call a data type 
_data-backed_ if it is representationally equivalent to [purple]#BuiltinData#. 
...
Now that Plutus core provides a way to deal directly with 
