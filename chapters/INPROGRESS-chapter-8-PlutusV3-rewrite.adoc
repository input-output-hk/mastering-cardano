...
instances for our custom type. We use template Haskell, that 
...
quotes in front of the type’s name to use it as an input variable. 
...
this transaction falls into the validity range of the transaction. 
...
In the examples we have seen so far, we had one specific validator
