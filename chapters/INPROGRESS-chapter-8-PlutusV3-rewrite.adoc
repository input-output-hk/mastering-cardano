...
features. The code from this section was tested with a Nix shell that was 
set up with instructions provided in the _plinth-dev-env_ repository. 
...
in this section. Below is an example blueprint for the above code. 
...
the transaction gets processed by the node only if the time of submission 
