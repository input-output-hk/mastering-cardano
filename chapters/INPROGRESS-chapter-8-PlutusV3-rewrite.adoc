...
the `CBORHEX` lengths decrease for around 10%. The table below shows compiled 
...
to typed form. We call that a lazy-decoding. This also brings an on-chain code 
performance advantage. 

The reason why compiled code is shorter if we import the [purple]#ScriptContext#
from the `PlutusLedgerApi.Data.V3` module is because that module provides an alternative 
interface that works directly with the [purple]#Data# type under the hood. Because 
...
for all Plutus script version (V1, V2 and V3), developers can move away from sum of 
...
in form of a _data-backed_ type. Also, one can look at the 
