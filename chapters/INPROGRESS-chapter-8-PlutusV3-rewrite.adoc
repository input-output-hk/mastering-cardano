...
from the script address. We convert it to `POSIX` time in milliseconds. 
After we have define our data that we will attach in the datum we can 
define our vesting script and compute the address of this script. The 
compiled code is shortened in our code snippet. When running this code 
the actual `CBORHEX` of the validator has to be provided that can be 
found in the `blueprint.json` file at the 
...
current time and substract 40 seconds. Then we define our transaction. 
We first set the network and then use the [blue]#redeemValue# 
...
parameterized vesting script. We shorten again the `CBORHEX` compiled 
code. The full code can be found in the `blueprint.json` file that 
gets created when we compile our Plinth validation code to Plutus. 
We use now the function [blue]#applyParamsToScript# and add the 
...
We make an important distinction between off-chain and on-chain code at the 
end. Because Cardano uses the hard fork combinator technology, all Plutus script 
versions are supported by the blockchain (read more in section <<Cardano node 
layers>>). Once you have writen on-chain code that works, it will work indefinitely. 
That is not the case for off-chain code. 
...
In the next section we will show how to write a minting policy and 
off-chain code that interacts with the policy.
