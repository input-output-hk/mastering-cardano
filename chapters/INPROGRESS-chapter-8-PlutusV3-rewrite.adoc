...
This chapter presents the MeshJS tool and showcases how to write
...
language embeded in Haskell which compiles to untyped Plutus core. Because 
...
Cardano ecosystem report] shows that Aiken is by far the most used smart contract language 
in Cardano. It is a user friendly standalone language with great documentation. 
The reason why the _Plutus smart contracts_ section focuses on Plinth is 
there are already many great learning resources for Aiken. Plinth as a high 
assurance language works better with with proof assistans as Agda, that compiles 
to Haskell and can be used for formal verification of smart contract code. 
A more detailed comparisson of Aiken and Plinth can be found in the 
<<Plinth in comparison to Aiken>> section. For developers that quickly want to learn 
how to code smart contracts in Cardano and would want to avoid learning 
Haskell, Aiken is a good choice. Aiken specific learning resources 
are presented at the end of section <<Plinth in comparison to Aiken>>. 
...
As stated in the begning Plinth is embedded in the Haskell programming language. 
...
the advantages they bring. _Plutus smart contracts_ section covers 
all together the following topics:
...
offered by https://education.emurgo.io/about-us/[Emurgo Academy]. At the 
time of writing, the offered courses include: 
...
courses] page provides an up-to-date list. The 
https://github.com/johnnygreeney/cardanoforthemasses[Cardano for the Masses]
online book written by John Green is also available under a open-source license. 
...
a new Plutus script version available. If you encouter Plinth code examples written 
in PlutusV1 or PlutusV2 you can checkout videos and learning examples of the previous 
...
A Plutus smart contract, also called a Plutus script, provides a more flexible 
view than a Bitcoin script but does not have a global view as a Solidity smart 
contract. Plutus scripts cannot see the whole state of the blockchain but can 
see the whole transaction that is being validated.

This means the Plutus script can see the script context that
carries information about the transaction being validated including 
its inputs and outputs and other things that will be presented in detail 
in section <<Script context explained>>. A UTXO can also have an arbitrary
...
Plutus, or more preceisly to UPLC which the Cardano nodes execute. Plinth 
provides a compiler plugin for GHC (the Glasgow Haskell Compiler) that compiles 
to Plutus. More details about the Plinth-to-Plutus compilation pipeline are 
outlined in section <<Plutus security>>. 
...
Aiken was developed by the company https://txpipe.io/[TxPipe] with
help of the https://cardanofoundation.org/[Cardano Foundation]. It takes
...
Aiken compiles all its custom data types into the builtin Plutus type [purple]#Data#.
We will look at this data type in section <<Simple validation scripts>>. 

Both languages are purely functional, which makes it easier for an auditor 
to assess the behavior of such smart contracts. Because Plinth is embeded in 
Haskell, it also works well with proof-assistant tools such as
...
writing smart contracts is very similar, even though the syntax is 
...
Aiken repository. For those wich prefer to learn by a book the 
https://book.io/book/i-can-aiken/[I can Aiken] book, written by John 
Green provides a lot of information and learning examples on Aiken. 
...
that he received in his pool. In reality this is hard to achieve since 
it is not known in advance which node will be elected to mint the next 
block. Also if the network has a high number of block producing nodes 
this helps to prevent front-running attacks. At the time of writing 
Cardano has around 3000 stake pools that operate on mainnet and because 
of Cardano's design none of them controls a significant amount of staked ada. 
You can read more about this topic in chapter 
<<Stake pools and stake pool operations>>. We also note that for transactions 
that interact with the same validator and spend different UTXOs, rearranging 
them is inconsequential because there is no global state that would affect 
both of the transaction. You can read more about the mempool and front-running 
invasion attacks on Cardano in the following 
