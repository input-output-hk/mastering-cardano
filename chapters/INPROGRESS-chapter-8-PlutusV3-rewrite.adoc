...
coding. We name the module `Week02.Validators` because the code comes 
from the _Week02_ examples of the fourth Plutus pioneer program. 
...
type we need to use the [purple]#makeIsDataSchemaIndexed# function that generates 
...
other sections you can use https://deno.com/[Deno] that is a runtime for 
JavaScript and TypeScript. All off-chain code presented in this and other 
sections was tested with Deno version 2.1.9 and MeshSDK version 1.9.0. 
To install the MeshSDK libraries that are needed by the off-chain code 
first install the `npm` package manager for JavaScript and then run: 
...
account. Currently it is set to 50.000. 

After we have defined our provider we initiate a Mesh wallet where we 
input the provider and our secreet seed. We also define on which network 
we will work (0 stands for testnet which is both preview or preprod). 
Mesh also allows to provide the key in other forms. One can define the  
type of the key as `"root"` and then provide the root key in `bech32` 
format. Or define the type of key as `"cli"` and then provide the 
payment and staking keys. To see all options you can look at the 
source code in the official mesh GitHub repository that defines the 
...
First we define the wallet adress and the public key hash that 
bellongs to that address. We will set the beneficiary to our own 
...
After we have define our data that we will attach in the datum we can 
...
After that we set the change address to our address which means that the 
lovelace change amout of the UTXOs we will spend will go back to our wallet. 
...
We imagine for a moment that we define the above data type as a data type 
that uses a logical `OR` such that it flips the order of the arguments. 

[source,haskell]
----
data VestingDatum = VestingDatum1 { beneficiary1 :: PubKeyHash, 
                                    deadline1 :: POSIXTime}
                  | VestingDatum2 { deadline2 :: POSIXTime, 
                                    beneficiary2 :: PubKeyHash}
----

In principle we could modify the validator code such that it can work with the 
updated [purple]#VestingDatum# type we have just defined. If we would do that, we 
would have then 2 options in our off-chain code how to define the datum. In the 
...
that coresponds to the second data constructor [purple]#VestingDatum2#. If our vesting 
type would have a thrid data constructor we could use the function [blue]#mConStr2#. 
The MeshJS library currently provides these functions up to [blue]#mConStr3#. 

After we have created our transaction we have to build it, sign it and submit it. 
In the end we return the transaction hash of the submitted transaction. Next we 
define the functions needed to claim the vested funds. 
...
then one data constructor we would state which data constructor we are 
...
current time and substract 40.000 milliseconds. Then we define our 
...
The redeemer is optional and could also be skipped. Same goes for 
...
We first define the list of imports that in addition containes the 
...
then one parameter, we would enter all of them in the list in the 
...
We define the [blue]#sendFunds# in a similar way as in the previous 
...
select our funds. We complete the transaction, singed and submit it 
...
The transaction to claim the funds is structured also in a similar way 
...
In the end we define the function calls by logging their return values. 
...
be too low and it depends on the script size we are deploying. In the 
end we claim the funds. There we have to input the transaction hashes 
that the previous two function calls returned. As in our previous off-chain 
code example we have to uncomment only one line at a time and execute the 
code. If we name our file `vesting-param.ts` we can again execute it with Deno: 
...
a unredeemable UTXO is that referencing a script from a UTXO is cheaper 
then attaching it to the transaction. Of course the upfront cost is 
...
In the end we also make an important distinction between off-chain and 
on-chain code. Because Cardano uses the hard fork combinator technology, 
all Plutus script versions are supported by the blockchain (read more in 
section <<Cardano node layers>>). Once you have writen on-chain code that 
works, it will work indefinitely. That is not the case for off-chain code. 

Because the ledger rules can be updated, it can happen that conditions which 
a transaction needs to fullfill may change. An example is the required ada fee 
...
[purple]#MintValue# type that is structured same way as the [purple]#Value# 
...
A minting policy requires similar as a spending validation script only 
...
parameterized with a currency symbol, then the coresponding minting policy 
...
First we add all the neccessary language pragmas. We then name the 
...
import all neccessary modules. Next we define our minting policy. It 
...
compile the minting policy as in the examples before and we serialize 
...
First we import all neccessary MeshJS classes and functions, then 
...
If the number would be negative, this would mean we are burning tokens. 
In that case, the wallet must already contain the tokens to be burned. 
We also note that in Plutus a token name can be an arbitrary byte string 
which is limited to 32 bytes. If a human-readable name is set, wallets also 
display it in that form. After setting the tokens we want to mint we define 
our minting function. In it we first define an empty redeemer. Next we create 
our transaction. In the transaction we set the network, say which tokens 
we want to mint with which minting policy, add the redeemer and sing the 
transaction with our wallet key. At the end we build, sign and submit the 
transaction and return the transaction hash. Now we can make our function 
call and mint the tokens. If we name our file `signed-minting.ts` we can 
mint our tokens as follows. 
...
it such that it takes in the transaction hash and output index as two seperate 
parameters. We do that in the [blue]#wrappedVal# helper function. It takes 
in four parameters that are all of type [purple]#BuiltinData# and converts them 
...
we coule also leave our policy such that it takes in the transaction output 
...
If a reader wants to know more about MeshJS one can look at the official: 

* https://meshjs.dev/guides[MeshJS guides] 
* https://docs.meshjs.dev/[documentation] 
* https://github.com/MeshJS/examples/tree/main
* https://pbl.meshjs.dev/course/mesh
