...
typed version with the use of the [blue]#fromBuiltinData# function. It takes in 
...
reason why we call it _unsafe_ because the conversion might fail. Below you can 
see the [purple]#FromData# and [purple]#UnsafeFromData# type classes. 
...
before we can compile our validator is has to be in untyped form. So we wrapp 
...
All together we have defined four different variant for the 42 validator. We 
...
two different Plutus ledger API modules we can use.  
...
Now that Plutus core provides a way to deal directly with the [purple]#Data# type 
...
As mentioned before, the untyped version has a performance advantage.
