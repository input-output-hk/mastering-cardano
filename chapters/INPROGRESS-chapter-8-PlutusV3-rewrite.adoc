...
module that we have imported. Below is an example blueprint for the above 
code. The extend the blueprint code such that it generates the data and 
compiled code for all six validators we have defined in this section. 
...
code resides and its build depends that lists Plutus packages needed by our validator 
...
The transaction information is only one and is accessible to to every script 
...
We also realy on the fact that the probability of a hash collision is negligible. 
...
For PlutusV3 scripts a datum is no longer neccessary to be attached to a UTXO 
...
The [purple]#Datum# type is same as the [purple]#Redeemer# tpye a wrapper 
around the [purple]#BuiltinData# type. The datum allows information being 
attached to a UTXO that can then be accessed withing the validation script 
...
The third option for a script purspose is rewarding that is used for 
...
* Voting and proposing - are used when government actions are invloved. 
...
[blue]#unsafeDataAsI# function to convert it to a integer number. 
...
Let us look at the at the transaction information [purple]#TxInfo# data type. 
...
that we have previously exaplained. It defines the input UTXO. And it also 
...
A transaction output of a UTXO contains the adress this UTXO is residing, 
...
The third part of a a transaction output is the [purple]#OutputDatum# data type. 
...
the constructor names are different and the Spending constructor holds only 
...
has passed. Such a contract can take two aproaches, depending on how the 
...
* From a parameter that is added as a input variable to the validator script. 

First we present the aproach when the validator uses the datum of the UTXO. 
...
In the begining we add all neccessary language pragmas and import 
statements. The functions, types and type classes we import are neccessary
to cover the functionality for both validators presented presented in this 
section. We name the module `Week03.Vesting` since the idea for the vesting 
contract comes from the _Week03_ examples from the Plutus pioneer program. 
...
the script context and returned a bool or a builtin unit. If there is any variability 
in the contract, we model that by using the datum as in the vesting example, that 
contained the beneficiary and the deadline. An alternative approach are parameterized 
contracts, where variability is integrated into the contract by adding a parameter 
variable to the validator function. The code below shows this, containing a modified 
version of the previous vesting code example. 
...
Template Haskell and generate the neccessary instances for our custom type. 
...
parameter is not know at the time of compilation. 
...
the beneficiaries public key hash from the verstin parameter. Apart from that 
...
This concludes the procedure of writing a parameterized contract. An example of 
applying more then one parameter is included in section <<Minting policies>>. 
