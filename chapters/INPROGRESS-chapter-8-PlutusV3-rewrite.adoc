...
on-chain code behave as expected instead of testing the off-chain code 
on the real test or main netwroks. From 
...
We call our testing approach _unified testing_ if one can reause the on-chain, 
off-chain and testing code accross different ledger backends. Before we show 
how to unit and property test a Plinth validator with the above mentioned tools, 
for a better understanding we demonstrate how to model and test a simple mock 
blockchain in Haskell using a State monad. First we look at how to represent 
a blockchain and keep track of transactions. 
...
* that the deadline contained in the datum, which is a custom data 
type, has passed.

Below, we can see the smart contract code, where we name our module
`Week06.NegativeRTime`:
