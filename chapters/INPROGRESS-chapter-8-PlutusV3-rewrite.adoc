...
the `CBORHEX` lenghts decrease for around 10%. The table below shows compiled 
code lengts for the four validators and for the two different `PlutusLedgerApi` 
modules we can use.  
...
type which allows to retain the user friendliness of the data type version while also 
...
and other Plutus datatypes contained in the [purple]#ScriptContext# in form 
of _data-backed_ types. Also one can look at the 
