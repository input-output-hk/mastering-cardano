...
MeshSDK packages all of version 1.9.0. To install the MeshSDK packages 
that are needed by the off-chain code one can use the `npm` package manager 
for JavaScript. The package that installs all other needed packages as 
dependencies is `@meshsdk/core`. The official 
https://www.npmjs.com/package/@meshsdk/core[npm page] for this package 
lists the MeshSDK packages that will get installed. To install them 
locally `cd` to the root location of your project and run: 
...
For more information on how to install npm packages locally or globally one can read the 
https://docs.npmjs.com/packages-and-modules/getting-packages-from-the-registry[Getting 
packages from the registry] npm documentation page.  
